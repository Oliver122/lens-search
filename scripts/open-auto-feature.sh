...
# Same CYCLE: keep existing auto-feature (re-run). New CYCLE: reset from main and force-push.
...
if git rev-parse --verify "refs/heads/${feat}" >/dev/null 2>&1 \
  ...
  if existing_cycle="$(cycle_on_ref "origin/${feat}")"; then
    existing_ref="origin/${feat}"
  fi
fi
...
if [[ -n "$existing_ref" && "$existing_cycle" == "$hash" ]]; then
  ...
  if [[ -n "${OPEN_AUTO_FEATURE_PUSH:-}" ]]; then
    if remote_has_branch "$feat"; then
      git push --force-with-lease -u origin "$feat"
    else
